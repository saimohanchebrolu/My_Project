...
router.post('/', async (req, res, next) => {
  ...
  try {
    ...
    // Lock only cart_items (PostgreSQL compatible)
    const cartResult = await client.query(
      `
      SELECT
          ci.product_id,
          ci.quantity,
          ci.delivery_option_id,
          p.price_cents,
      ...
      LEFT JOIN delivery_options d
        ON d.id = ci.delivery_option_id
    ...
    const totalBeforeTaxCents =
      itemsSubtotalCents + shippingCents;

    const taxCents =
      Math.round(totalBeforeTaxCents * TAX_RATE);
    ...
    for (const row of cartResult.rows) {
      ...
      const estimatedDeliveryDate =
        addDays(now, row.days_to_deliver);
